...
- That work (paid phone verification + mutual contacts, session scott-mcp-c2) is now committed and **merged with mine**: rf `lc-sdk` = rust `e46629c`, dart `fa47e5f` ("Merge rf/lc-sdk (directory-bound short id) into the contacts/phone work"). The lc_identity.rs conflict (my removal of the `short_id` field vs. their new `Service` struct) was resolved by that session.
